...
export const getBadgeRequestById = async (req, res) => {
  try {
    ...
    if (!request) return res.status(404).json({ error: "Badge request not found" });
    res.json(request);
  } catch (error) {
...
export const updateBadgeRequestStatus = async (req, res) => {
  try {
    ...
    const updatedRequest = await BadgeRequest.updateStatus(id, status, reviewedBy, adminNotes);
